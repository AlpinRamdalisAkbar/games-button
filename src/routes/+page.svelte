...
<div class="relative flex justify-center items-center h-dvh bg-gray-800">
    {#if visible}
            <!-- triangle  -->
            <svg in:fly={{ x: 0, y: 300, duration: 600 }} 
                out:fly={{ x: 0, y: 300, duration: 600 }} 
                class="absolute top-10 w-20 h-20 drop-shadow-[0_0_10px_#00FF00]" viewBox="0 0 24 24">
                <polygon points="12,4 4,20 20,20" fill="none" stroke="#00FF00" stroke-width="2" />
            </svg>
            
            <!-- Square (Left) -->
            <svg in:fly={{ x: 800, y: 0, duration: 600 }} 
                out:fly={{ x: 800, y: 0, duration: 600 }} 
                class="absolute left-10 w-20 h-20 drop-shadow-[0_0_10px_#FF69B4]" viewBox="0 0 24 24">
            ...
            <svg in:fly={{ x: -800, y: 0, duration: 600 }} 
                out:fly={{ x: -800, y: 0, duration: 600 }} 
                class="absolute right-10 w-20 h-20 drop-shadow-[0_0_10px_#FF0000]" viewBox="0 0 24 24">
            ...
            <svg in:fly={{ x: 0, y:-300, duration: 600 }} 
                out:fly={{ x: 0, y:-300, duration: 600 }}
                class="absolute bottom-10 w-20 h-20 drop-shadow-[0_0_10px_#00BFFF]" viewBox="0 0 24 24">
    ...
    <button
        ...
        class={`z-10 duration-300 ease-in pt-5 pb-5 pr-10 pl-10 rounded-2xl text-gray-300 bg-gray-700 flex items-center gap-3
                ${glowClass(glowDirection)}`}>
        <img src="./playstation-svgrepo-com.svg" alt="PlayStation Logo" class="w-6 h-6" />
        Let's Play
    </button>
</div>
<style lang="postcss">
